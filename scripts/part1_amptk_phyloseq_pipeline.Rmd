...
```{zsh, engine.opts='-l'}
source ~/.zshrc
amptk-docker taxonomy -f clustered.cluster.otus.fa -i clustered.otu_table.txt -m trimmed.mapping_file.txt -d ITS -o Westside_Toby
```
...
mv -v trimm* cluster* Westside_Toby* data/intermediate_data/amptk/ 2>/dev/null

mv -v data/intermediate_data/amptk/*.log data/intermediate_data/amptk/logs/ 2>/dev/null

mv -v data/intermediate_data/amptk/Westside_Toby* data/intermediate_data/amptk/otu_tables/ 2>/dev/null
```
...
biom_file <- "data/intermediate_data/amptk/otu_tables/Westside_Toby.biom"
...
saveRDS(RARE, file = "data/processed_data/phyloseq/tobybregar_phyloseq_rare.rds")
```
